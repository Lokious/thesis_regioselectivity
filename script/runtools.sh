...
for group in "${Domain[@]}"; do
	wait
	cmd=`hmmsearch --domE 0.00001 -E 0.00001 --domtblout ../autodata/align/${group}_domout.tsv --tblout ../autodata/align/${group}_tbout.tsv --cpu 4 ../autodata/align/${group}.hmm ../autodata/rawdata/uniprot_ec2.1.1.fasta`
	echo $cmd
done
echo `python3 hmm.py`
for group in "${Domain[@]}"; do
