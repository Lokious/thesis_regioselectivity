...
def hhalign(domian_list):
    ...
    while domian_list:
        ...
        hhalign_cmd = "hhalign -i ../autodata/align/separate_by_domain/no_overlap_sequences/{0}_hmmalign_out_trim.a2m -t ../autodata/align/separate_by_domain/no_overlap_sequences/{1}_hmmalign_out_trim.a2m -oa3m ../autodata/align/separate_by_domain/no_overlap_sequences/{2}".format(domain_new,
                                                               template,
                                                               out_align)
        os.system(hhalign_cmd)
        log_file.write(hhalign_cmd)
        template = out_align
    print(out_align)
    log_file.close()
    return out_align
    ...
    for domain in domains:
        ...
        os.system(
            "hmmfetch ../autodata/align/Pfam35.0/Pfam-A.hmm {0} > ../autodata/align/{0}.hmm".format(
                domain))
        os.system("wait")
        cmd1 = 'hmmalign --amino --trim  --outformat A2M ../autodata/align/{0}.hmm ../autodata/sequences/no_overlap_sequences/{0}.fasta > ../autodata/align/separate_by_domain/no_overlap_sequences/{0}_hmmalign_out_trim.a2m'.format(
            domain)
